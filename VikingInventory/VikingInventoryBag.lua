...
local knDefaultAltCurrency = 6 -- index into karCurrency for Omnibits
...
function InventoryBag:OnDocumentReady()
	...
	self.wndMain:FindChild("OptionsConfigureCurrencyList"):ArrangeChildrenVert(Window.CodeEnumArrangeOrigin.LeftOrTop)
...
function InventoryBag:OnInterfaceMenuListHasLoaded()
	Event_FireGenericEvent("InterfaceMenuList_NewAddOn", Apollo.GetString("InterfaceMenu_Inventory"), {"InterfaceMenu_ToggleInventory", "VikingInventory", "Icon_Windows32_UI_CRB_InterfaceMenu_Inventory"})
end
...
function InventoryBag:OnToggleVisibility()
	...
	if self.wndMain:IsShown() then
		...
		self:OnQuestObjectiveUpdated() -- Populate Virtual VikingInventory Btn from reloadui/load
...
function InventoryBag:HelperGetCurrencyAmmount(tData)
	local monAmount = 0
	if tData.bAccountItem then
...
function InventoryBag:OnDragDropNotifyTrash(wndHandler, wndControl, bMe) -- TODO: We can probably replace this with a button mouse over state
	if bMe then
		self.wndMain:FindChild("TrashIcon"):SetSprite("CRB_Basekit:kitIcon_NewDisabled")
