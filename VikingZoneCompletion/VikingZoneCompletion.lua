-----------------------------------------------------------------------------------------------
-- Client Lua Script for VikingZoneCompletion
...
function VikingZoneCompletion:new(o)
    o = o or {}
    setmetatable(o, self)
    self.__index = self
    return o
...
function VikingZoneCompletion:OnSave(eType)
...
function VikingZoneCompletion:OnRestore(eType, tSavedData)
	if eType ~= GameLib.CodeEnumAddonSaveLevel.Character then
		return
	end
...
function VikingZoneCompletion:Init()
    Apollo.RegisterAddon(self)
end

function VikingZoneCompletion:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("VikingZoneCompletion.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)
end

function VikingZoneCompletion:OnDocumentReady()
...
function VikingZoneCompletion:OnClose(wndHandler, wndControl)
	Apollo.StopTimer("ZoneCompletion_UpdateWindowTimer")
...
function VikingZoneCompletion:OnMinimized(wndHandler, wndControl)
...
function VikingZoneCompletion:OnOpened(wndHandler, wndControl)
...
function VikingZoneCompletion:OnZoneChanged(nZoneId)
	if self.wndMain and self.wndMain:IsValid() then
...
function VikingZoneCompletion:OnZoomLevelChanged(bValidZoneZoom)
	if self.wndMain and self.wndMain:IsValid() then
...
function VikingZoneCompletion:OnSetMapCompletionShown(bToggle)
	if self.wndMinimizeToggle and self.wndMinimizeToggle:IsValid() then
		self.wndMinimizeToggle:FindChild("ZoneCompletionMinimizeToggleBtn"):SetCheck(bToggle)
	end
end

function VikingZoneCompletion:RedrawFromUI(wndHandler, wndControl)
	self:RedrawAll()
end
...
function VikingZoneCompletion:HelperBuildPercentExploredItem(nPercent)
	local wndCount = Apollo.LoadForm(self.xmlDoc, "CountItem", self.wndMain:FindChild("CountsContainer"), self)
...
function VikingZoneCompletion:HelperBuildPathCountItem(tPathValues)
...
local VikingZoneCompletionInst = VikingZoneCompletion:new()
VikingZoneCompletionInst:Init()
