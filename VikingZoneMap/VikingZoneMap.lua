-------------------------------------------------------------------------------------------
-- Client Lua Script for VikingZoneMap
...
local kstrQuestFont 					= "CRB_Interface9"
...
function VikingZoneMap:CreateOverlayObjectTypes()
...
function VikingZoneMap:CreatePOIIcons()
	self.tPOITypes =
	{
...
function VikingZoneMap:GetAllUnitTypes()
	local tUnitTypes =
	{
...
function VikingZoneMap:BuildCustomMarkerInfo()
...
function VikingZoneMap:BuildShownTypesArrays()
...
function VikingZoneMap:Init()
	Apollo.RegisterAddon(self)
end

function VikingZoneMap:OnSave(eType)
...
function VikingZoneMap:OnRestore(eType, tSavedData)
	if eType ~= GameLib.CodeEnumAddonSaveLevel.Character then
		return
	end
...
function VikingZoneMap:OnLoad()
	Apollo.RegisterEventHandler("UnitCreated", 							"OnPreloadUnitCreated", self)

	self.xmlDoc = XmlDoc.CreateFromFile("VikingZoneMapForms.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)

	Apollo.RegisterEventHandler("InterfaceMenuListHasLoaded", "OnInterfaceMenuListHasLoaded", self)
end

function VikingZoneMap:OnPreloadUnitCreated(unitNew)
	self.arPreloadUnits[unitNew:GetId()] = unitNew
end

function VikingZoneMap:OnDocumentReady()
	...
	self.wndZoneMap 			= self.wndMain:FindChild("VikingZoneMap")
...
function VikingZoneMap:OnWindowManagementReady()
	Event_FireGenericEvent("WindowManagementRegister", {strName = Apollo.GetString("ZoneMap_CityDirections")})
end

function VikingZoneMap:OnInterfaceMenuListHasLoaded()
...
function VikingZoneMap:OnOptionsUpdated()
...
function VikingZoneMap:CreateUnitsFromPreload()
...
function VikingZoneMap:ToggleWindow()
...
function VikingZoneMap:UpdateRapidTransportBtn()
...
function VikingZoneMap:OnToggleGhostModeMap() -- for keyboard input turning ghost mode map on/off
	local bShow = not self.wndZoneMap:IsShowingGhostWindow()
	self.wndZoneMap:SetGhostWindow(bShow)
end

function VikingZoneMap:UpdateCurrentZone()
...
function VikingZoneMap:OnWindowMove(wndHandler, wndControl)
...
function VikingZoneMap:OnZoneMapChange(wndHandler, wndControl, strContinent, idZone) -- Map changed from clicking in continent view
	self.wndMain:FindChild("ZoneComplexToggle"):SetData(idZone) -- No need to set Continent since it has to be the current one
end

function VikingZoneMap:OnZoneComplexListBtn(wndHandler, wndControl) -- ZoneComplexListBtn
...
function VikingZoneMap:OnContinentNormalBtn(wndHandler, wndControl)
...
function VikingZoneMap:OnContinentCustomBtn(wndHandler, wndControl)
...
function VikingZoneMap:OnShowLoc(tLocInfo) -- Also from City Directions
...
function VikingZoneMap:OnWorldLocTimer()
	self.wndMain:ToFront()
end
...
function VikingZoneMap:OnCloseBtn()
	self.wndMain:Close()
	self.wndMapControlPanel:Show(false)
	self.wndMain:FindChild("ZoneComplexList"):Show(false)
	self.eDisplayMode = self.wndZoneMap:GetDisplayMode()
...
function VikingZoneMap:OnWindowClosed()
...
function VikingZoneMap:OnWorldChanged()
...
function VikingZoneMap:ToggleGhostModeOn()
	self.wndZoneMap:SetGhostWindow(true)
	Event_FireGenericEvent("MapGhostMode", true)
	self:ToggleWindow()
end

function VikingZoneMap:OnGhostSlider(wndHandler, wndControl)
...
function VikingZoneMap:OnGhostModeOptionOK()
	self.wndGhostOptionPanel:Show(false)
end

function VikingZoneMap:OnGhostModeOptionTest()
	self.wndZoneMap:SetGhostWindow(true)
	Event_FireGenericEvent("MapGhostMode", true)
	self:ToggleWindow()
end

function VikingZoneMap:OnToggleControlsOn(wndHandler, wndControl)
	...
	self.wndMain:FindChild("ToggleControlsBtn"):SetAnchorOffsets(-377, nTop, 0, nBottom)
	self.wndMain:FindChild("ToggleControlsBtn"):SetTooltip(Apollo.GetString("CRB_Collapse_zone_selection_controls"))
end

function VikingZoneMap:OnToggleControlsOff(wndHandler, wndControl)
	...
	local nLeft, nTop, nRight, nBottom = self.wndMain:FindChild("ToggleControlsBtn"):GetAnchorOffsets()
	self.wndMain:FindChild("ToggleControlsBtn"):SetAnchorOffsets(-50, nTop, 1, nBottom)
	self.wndMain:FindChild("ToggleControlsBtn"):SetTooltip(Apollo.GetString("CRB_Expand_zone_selection_controls"))

end

function VikingZoneMap:OnResizeOptionsPane()
...
function VikingZoneMap:SetControls() -- runs off timer, sets the controls to reflect the map's display
	...
	if eZoomLevel == tZoneMapEnums.World and tHomeContinent then
		...
		for idx = 1, #self.tContButtons do
			...
			if wndCurrentNavpoint then
				wndCurrentNavpoint:Show(tNavPoint and self.tContButtons[idx]:GetData() == tNavPointZoneInfo.continentId)
			end
		end
		self.wndZoneMap:SetZone(tHomeZone.id)
		self.wndMain:FindChild("ZoneComplexToggle"):SetData(tHomeZone.id)
...
function VikingZoneMap:HelperCheckAndBuildSubzones(tZoneInfo, eZoomLevel) -- This repeatedly calls on a timer
...
function VikingZoneMap:OnZoomChange()
...
function VikingZoneMap:OnMouseScroll(eDisplayMode)
...
function VikingZoneMap:OnZoomIn()
...
function VikingZoneMap:OnZoomOut()
	local eZoomLevel = self.wndZoneMap:GetDisplayMode() -- 1: Panning, 2: Scaled, 3: Continent
	if eZoomLevel == ZoneMapWindow.CodeEnumDisplayMode.SuperPanning then -- Super Panning
		self.wndZoneMap:SetDisplayMode(ZoneMapWindow.CodeEnumDisplayMode.Panning)
	elseif eZoomLevel == ZoneMapWindow.CodeEnumDisplayMode.Panning then -- Panning
...
function VikingZoneMap:OnReturnBtn(wndHandler, wndControl)
...
function VikingZoneMap:OnQuestStateChanged()
...
function VikingZoneMap:AddQuestIndicators()
...
function VikingZoneMap:GetMissionTooltip(pmCurrent)
...
function VikingZoneMap:GetPublicEventTooltip(peEvent)
...
function VikingZoneMap:GetChallengeTooltip(oChallenge)
...
function VikingZoneMap:GetHexGroupTooltip(oHexGroup)
...
function VikingZoneMap:GetNemesisRegionTooltip(oNemesisRegion)
...
function VikingZoneMap:OnGenerateTooltip(wndHandler, wndControl, eType, nX, nY)
	local eZoomLevel = self.wndZoneMap:GetDisplayMode()
...
function VikingZoneMap:OnHazardShowMinimapUnit(idHazard, unitHazard, bIsBeneficial)
...
function VikingZoneMap:OnHazardRemoveMinimapUnit(idHazard, unitHazard)
	if unitHazard == nil then
		return
	end
...
function VikingZoneMap:OnUpdateHexGroup(hexGroup)
...
function VikingZoneMap:OnPlayerIndicatorUpdated()
	self:SetControls()
end

function VikingZoneMap:OnSubZoneChanged()
	self:OnZoomChange()
...
function VikingZoneMap:OnToggleLabels(wndHandler, wndControl)
...
function VikingZoneMap:OnTerrainHexCheck(wndHandler, wndControl)
	wndHandler:FindChild("Label"):SetTextColor(ApolloColor.new("UI_BtnTextGoldListNormal"))
	Apollo.SetConsoleVariable("draw.hexGrid", true)
end

function VikingZoneMap:OnTerrainHexUncheck(wndHandler, wndControl)
	wndHandler:FindChild("Label"):SetTextColor(ApolloColor.new("UI_TextMetalGoldHighlight"))
	Apollo.SetConsoleVariable("draw.hexGrid", false)
end

function VikingZoneMap:SetTypeVisibility(eToggledType, bVisible)
...
function VikingZoneMap:OnMarkerBtnCheck(wndHandler, wndControl)
...
function VikingZoneMap:OnMarkerBtnUncheck(wndHandler, wndControl)
	local eType = wndHandler:GetData()
	if eType == ktMarkerCategories.QuestNPCs then
...
function VikingZoneMap:OnMissionsCheck(wndHandler, wndControl)
...
function VikingZoneMap:OnChallengesCheck(wndHandler, wndControl)
	if wndControl ~= nil then  -- nil if coming from map change
...
function VikingZoneMap:OnPublicEventsCheck(wndHandler, wndControl)
	if wndControl ~= nil then  -- nil if coming from map change
...
function VikingZoneMap:ReloadNemesisRegions()
...
function VikingZoneMap:OnMissionsUncheck(wndHandler, wndControl)
	if wndControl ~= nil then  -- nil if coming from map change
...
function VikingZoneMap:OnChallengesUncheck(wndHandler, wndControl)
	if wndControl ~= nil then  -- nil if coming from map change
...
function VikingZoneMap:OnPublicEventsUncheck(wndHandler, wndControl)
	if wndControl ~= nil then  -- nil if coming from map change
...
function VikingZoneMap:RehideAllToggledIcons()
...
function VikingZoneMap:OnUnitChanged(unitUpdated, eType)
...
function VikingZoneMap:OnUnitDestroyed(unitDead)
...
function VikingZoneMap:IsTypeCurrentlyHidden(objectType)
	local eZoomLevel = self.wndZoneMap:GetDisplayMode()
	if eZoomLevel == ZoneMapWindow.CodeEnumDisplayMode.SuperPanning then
...
function VikingZoneMap:GetDefaultUnitInfo()
	local tInfo =
	{
		strIcon 		= "",
		strIconEdge 	= "",
...
function VikingZoneMap:GetOrderedMarkerInfos(tMarkerStrings)
	local tMarkerInfos = {}
...
function VikingZoneMap:OnUnitCreated(unitMade)
...
function VikingZoneMap:OnUnitCreateDelayTimer()
...
function VikingZoneMap:OnUnitDelayedCreated(unitMade)
...
---------------------------------------------------------------------------------------------------
-- Challenges
function VikingZoneMap:OnFailChallenge(tChallengeData)
	self:OnRemoveChallengeIcon(tChallengeData:GetId())
	self:UpdateChallengeList()
end

function VikingZoneMap:OnRemoveChallengeIcon(idChallenge)
...
function VikingZoneMap:OnAddChallengeIcon(chalCurrent, strPointIcon)
...
function VikingZoneMap:OnFlashChallengeIcon(idChallenge, strDescription, fDuration, tPosition)
...
function VikingZoneMap:OnStopChallengeFlashIcon()
	Apollo.StopTimer("ChallengeFlashIconTimer")
...
function VikingZoneMap:UpdateChallengeList()
...
function VikingZoneMap:ChallengeEntryMouseEnter( wndHandler, wndControl, x, y )
...
function VikingZoneMap:ChallengeEntryMouseExit( wndHandler, wndControl, x, y )
	if wndHandler:GetId() ~= wndControl:GetId() then
		return
	end
...
function VikingZoneMap:ChallengeEntryMouseButtonDown( wndHandler, wndControl, eMouseButton, nLastRelativeMouseX, nLastRelativeMouseY, bDoubleClick, bStopPropagation )
	if wndHandler:GetId() ~= wndControl:GetId() then
		return
	end
...
---------------------------------------------------------------------------------------------------
-- Missions
function VikingZoneMap:OnPlayerPathMissionActivate(pmActivated)
...
function VikingZoneMap:OnPlayerPathMissionDeactivate(pmEnded)
...
function VikingZoneMap:UpdateMissionList()
	if self.wndMain == nil then
		-- yes, it is possible for this to be nil here because we might not have gotten the OnLoad event yet
		return
	end
...
function VikingZoneMap:MissionEntryMouseEnter( wndHandler, wndControl, x, y )
	if wndHandler:GetId() ~= wndControl:GetId() then
		return
	end
...
function VikingZoneMap:MissionEntryMouseExit( wndHandler, wndControl, x, y )
	if wndHandler:GetId() ~= wndControl:GetId() then
		return
	end
...
function VikingZoneMap:MissionEntryMouseButtonDown( wndHandler, wndControl, eMouseButton, nLastRelativeMouseX, nLastRelativeMouseY, bDoubleClick, bStopPropagation )
	if wndHandler:GetId() ~= wndControl:GetId() then
		return
	end
...
---------------------------------------------------------------------------------------------------
-- Public events
function VikingZoneMap:OnPublicEventUpdate(peUpdated)
...
function VikingZoneMap:OnPublicEventCleared(peCurrent)
...
function VikingZoneMap:OnPublicEventEnd(peEnding, eReason, tEventInfo)
...
function VikingZoneMap:OnPublicEventObjectiveUpdate(peoUpdated)
	if not self.tToggledIcons.bPublicEvents then
		return
	end
...
function VikingZoneMap:OnPublicEventObjectiveEnd(peoEnding)
...
function VikingZoneMap:OnMapPulseTimer()
...
function VikingZoneMap:UpdatePublicEventList()
	if self.wndMain == nil then
		-- yes, it is possible for this to be nil here because we might not have gotten the OnLoad event yet
		return
	end
...
function VikingZoneMap:PublicEventEntryMouseEnter( wndHandler, wndControl, x, y )
	if wndHandler:GetId() ~= wndControl:GetId() then
		return
	end
...
function VikingZoneMap:PublicEventEntryMouseExit( wndHandler, wndControl, x, y )
	if wndHandler:GetId() ~= wndControl:GetId() then
		return
	end
...
function VikingZoneMap:PublicEventEntryMouseButtonDown( wndHandler, wndControl, eMouseButton, nLastRelativeMouseX, nLastRelativeMouseY, bDoubleClick, bStopPropagation )
	if wndHandler:GetId() ~= wndControl:GetId() then
		return
	end
...
function VikingZoneMap:OnGroupJoin()
	for idx = 2, GroupLib.GetMemberCount() do
		local tInfo = GroupLib.GetGroupMember(idx)
		if tInfo.bIsOnline then
			self.tGroupMembers[idx] =
...
function VikingZoneMap:OnGroupAdd(strName)
	for idx = 2, GroupLib.GetMemberCount() do
		local tInfo = GroupLib.GetGroupMember(idx)
...
function VikingZoneMap:OnGroupRemove(strName, eReason)
...
function VikingZoneMap:OnGroupLeft(eReason)
	for idx, tMember in pairs(self.tGroupMembers) do -- remove all of the group objects
		self.wndZoneMap:RemoveObject(tMember.mapObject)
	end
...
function VikingZoneMap:OnGroupUpdatePosition(arMembers)
...
function VikingZoneMap:DrawGroupMembers()
	for idx = 2, GroupLib.GetMemberCount() do
...
function VikingZoneMap:DrawGroupMember(tMember)
...
-----------------------------------------------------------------------------------------
-- VikingZoneMap Mouse Move and Click
-----------------------------------------------------------------------------------------

function VikingZoneMap:OnZoneMapButtonDown(wndHandler, wndControl, eButton, nX, nY, bDoubleClick) -- TODO: Bulletproof this
...
function VikingZoneMap:OnZoneMapMouseMove(wndHandler, wndControl, nX, nY)
...
function VikingZoneMap:OnZoneMap_MapCoordinateDelay()
	Apollo.StopTimer("ZoneMap_MapCoordinateDelay")
	self.bMapCoordinateDelay = false
end
...
function VikingZoneMap:OnCityDirectionsList(tDirections)
...
function VikingZoneMap:OnCityDirectionsClosed(wndHandler, wndControl)
	if self.wndCityDirections ~= nil and self.wndCityDirections:IsValid() then
		self.wndCityDirections:Close()
	end
end

function VikingZoneMap:OnCityDirectionsWindowClosed(wndHandler, wndControl)
	Event_CancelCityDirections()
	self.wndCityDirections:Destroy()
	self.wndCityDirections = nil
end

function VikingZoneMap:OnCityDirectionsClosedBtnSignal(wndHandler, wndControl)
	if self.wndCityDirections ~= nil and self.wndCityDirections:IsValid() then
		self.wndCityDirections:Close()
	end
end
...
function VikingZoneMap:OnCityDirectionMarked(tLocInfo)
...
function VikingZoneMap:OnZoneMap_PollCityDirectionsMarker()
...
function VikingZoneMap:OnZoneMap_TimeOutCityDirectionMarker()
	self.tCityDirectionsLoc = nil
	Apollo.StopTimer("ZoneMap_PollCityDirectionsMarker")
	Apollo.StopTimer("ZoneMap_TimeOutCityDirectionMarker")
...
function VikingZoneMap:UpdateQuestList()
	if self.wndMain == nil then
		-- yes, it is possible for this to be nil here because we might not have gotten the OnLoad event yet
		return
	end
...
function VikingZoneMap:BuildQuestTitleString(queCurr)
	local strName = ""
...
---------------------------------------------------------------------------------------------------
function VikingZoneMap:BuildQuestTooltip(queCurr)
...
---------------------------------------------------------------------------------------------------
function VikingZoneMap:QuestEntryMouseEnter(wndHandler, wndControl)
	if wndHandler:GetId() ~= wndControl:GetId() then
		return
	end
...
---------------------------------------------------------------------------------------------------
function VikingZoneMap:QuestEntryMouseExit(wndHandler, wndControl)
	if wndHandler:GetId() ~= wndControl:GetId() then
		return
	end
...
function VikingZoneMap:QuestEntryMouseButtonDown(wndHandler, wndControl)
	if wndHandler:GetId() ~= wndControl:GetId() then
		return
	end
...
function VikingZoneMap:OnZoneMap_OpenMapToQuest(queArg)
...
function VikingZoneMap:ToggleActiveRegion(objUserData)
...
---------------------------------------------------------------------------------------------------
function VikingZoneMap:OnMapTrackedUnitUpdate(idTrackedUnit, tPos)
...
function VikingZoneMap:OnMapTrackedUnitDisable(idTrackedUnit)
	self.wndZoneMap:RemoveObjectsByUserData(self.eObjectTypeMapTrackedUnit, idTrackedUnit)
end

---------------------------------------------------------------------------------------------------
function VikingZoneMap:OnNavPointCleared()
	if not self.wndZoneMap or not self.wndZoneMap:IsValid() then
		return
	end
...
function VikingZoneMap:OnNavPointSet(tLoc)
...
function VikingZoneMap:OnSetNavPointFailed(tDetails)
...
function VikingZoneMap:SetNavPoint(nX, nY)
...
---------------------------------------------------------------------------------------------------
function VikingZoneMap:OnLevelChanged(level)
	if level == nil then
		return
	end
...
function VikingZoneMap:HelperGoToMap(idWorldZone)
	if not self.wndMain:IsVisible() then
		self:ToggleWindow()
	end
...
function VikingZoneMap:OnLevelUpUnlock_WorldMapAdventure_Astrovoid()
	self:HelperGoToMap(GameLib.MapZone.Illium)
end

function VikingZoneMap:OnLevelUpUnlock_WorldMapAdventure_Galeras()
	self:HelperGoToMap(GameLib.MapZone.Whitevale)
end

function VikingZoneMap:OnLevelUpUnlock_WorldMapAdventure_Hycrest()
	self:HelperGoToMap(GameLib.MapZone.Thayd)
end

function VikingZoneMap:OnLevelUpUnlock_WorldMapAdventure_Malgrave()
	self:HelperGoToMap(GameLib.MapZone.Grimvault)
end

function VikingZoneMap:OnLevelUpUnlock_WorldMapAdventure_NorthernWilds()
	self:HelperGoToMap(GameLib.MapZone.Whitevale)
end

function VikingZoneMap:OnLevelUpUnlock_WorldMapAdventure_Whitevale()
	self:HelperGoToMap(GameLib.MapZone.Wilderrun)
end

function VikingZoneMap:OnLevelUpUnlock_WorldMapDungeon_UltimateProtogames()
	self:HelperGoToMap(GameLib.MapZone.Malgrave)
end

function VikingZoneMap:OnLevelUpUnlock_WorldMapDungeon_SwordMaiden()
	self:HelperGoToMap(GameLib.MapZone.Wilderrun)
end

function VikingZoneMap:OnLevelUpUnlock_WorldMapDungeon_Skullcano()
	self:HelperGoToMap(GameLib.MapZone.Whitevale)
end

function VikingZoneMap:OnLevelUpUnlock_WorldMapDungeon_KelVoreth()
	self:HelperGoToMap(GameLib.MapZone.Auroria)
end

function VikingZoneMap:OnLevelUpUnlock_WorldMapDungeon_Stormtalon()
	self:HelperGoToMap(GameLib.MapZone.Galeras)
end

function VikingZoneMap:OnLevelUpUnlock_WorldMapDungeon_ProtogamesAcademyExile()
...
function VikingZoneMap:OnLevelUpUnlock_WorldMapDungeon_ProtogamesAcademyDominion()
	local tCurrentZoneInto = GameLib.GetCurrentZoneMap()
	if tCurrentZoneInto == nil then
		return
	end
...
function VikingZoneMap:OnRapidTransportOpen()
	self:OnCloseBtn()
	Event_FireGenericEvent("InvokeTaxiWindow")
end

function VikingZoneMap:OnLevelUpUnlock_WorldMapCapital_Thayd()
	self:HelperGoToMap(GameLib.MapZone.Thayd)
end

function VikingZoneMap:OnLevelUpUnlock_WorldMapCapital_Illium()
	self:HelperGoToMap(GameLib.MapZone.Illium)
end

function VikingZoneMap:OnLevelUpUnlock_WorldMapNewZone_Algoroc()
	self:HelperGoToMap(GameLib.MapZone.Algoroc)
end

function VikingZoneMap:OnLevelUpUnlock_WorldMapNewZone_Auroria()
	self:HelperGoToMap(GameLib.MapZone.Auroria)
end

function VikingZoneMap:OnLevelUpUnlock_WorldMapNewZone_Celestion()
	self:HelperGoToMap(GameLib.MapZone.Celestion)
end

function VikingZoneMap:OnLevelUpUnlock_WorldMapNewZone_CrimsonIsle()
	self:HelperGoToMap(GameLib.MapZone.CrimsonIsle)
end

function VikingZoneMap:OnLevelUpUnlock_WorldMapNewZone_Deradune()
	self:HelperGoToMap(GameLib.MapZone.Deradune)
end

function VikingZoneMap:OnLevelUpUnlock_WorldMapNewZone_Ellevar()
	self:HelperGoToMap(GameLib.MapZone.Ellevar)
end

function VikingZoneMap:OnLevelUpUnlock_WorldMapNewZone_EverstarGrove()
	self:HelperGoToMap(GameLib.MapZone.EverstarGrove)
end

function VikingZoneMap:OnLevelUpUnlock_WorldMapNewZone_Farside()
	self:HelperGoToMap(GameLib.MapZone.Farside)
end

function VikingZoneMap:OnLevelUpUnlock_WorldMapNewZone_Galeras()
	self:HelperGoToMap(GameLib.MapZone.Galeras)
end

function VikingZoneMap:OnLevelUpUnlock_WorldMapNewZone_Grimvault()
	self:HelperGoToMap(GameLib.MapZone.Grimvault)
end

function VikingZoneMap:OnLevelUpUnlock_WorldMapNewZone_LevianBay()
	self:HelperGoToMap(GameLib.MapZone.LevianBay)
end

function VikingZoneMap:OnLevelUpUnlock_WorldMapNewZone_Malgrave()
	self:HelperGoToMap(GameLib.MapZone.Malgrave)
end

function VikingZoneMap:OnLevelUpUnlock_WorldMapNewZone_NorthernWilds()
	self:HelperGoToMap(GameLib.MapZone.NorthernWilds)
end

function VikingZoneMap:OnLevelUpUnlock_WorldMapNewZone_Whitevale()
	self:HelperGoToMap(GameLib.MapZone.Whitevale)
end

function VikingZoneMap:OnLevelUpUnlock_WorldMapNewZone_Wilderrun()
	self:HelperGoToMap(GameLib.MapZone.Wilderrun)
end

---------------------------------------------------------------------------------------------------
function VikingZoneMap:FactoryProduce(wndParent, strFormName, tObject)
...
local ZoneMap_Singleton = VikingZoneMap:new()
